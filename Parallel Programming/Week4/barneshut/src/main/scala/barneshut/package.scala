...
package object barneshut {
  ...
  case class Fork(
    ...
    val (mass, total) = quadList.foldRight((0f, 0)) { case (child, (_mass, _total)) =>
      (child.mass + _mass, child.total + _total)
    }

    val massX: Float = {
      if (mass == 0) centerX
      else quadList.foldRight(0f)((child, _massX) => child.mass * child.massX + _massX) / mass
    }
    val massY: Float = {
      if (mass == 0) centerY
      else quadList.foldRight(0f)((child, _massY) => child.mass * child.massY + _massY) / mass
    }
  ...
  case class Leaf(centerX: Float, centerY: Float, size: Float, bodies: coll.Seq[Body])
    ...
    def insert(b: Body): Quad = {
      ...
      else _insert(xs.toSeq, b, size / 2, size / 4)
    }
  }
