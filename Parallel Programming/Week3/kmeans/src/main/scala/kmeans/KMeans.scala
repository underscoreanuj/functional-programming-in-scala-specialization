...
import scala.collection.{GenSeq, Map, Seq, mutable}
...
class KMeans extends KMeansInterface {
  ...
  def classify(points: Seq[Point], means: Seq[Point]): Map[Point, Seq[Point]] = {
    val pointsGroupByMeans = points.groupBy(findClosest(_, means))
    means.map(mean => (mean, pointsGroupByMeans.getOrElse(mean, Seq()))).toMap
  }

  def classify(points: ParSeq[Point], means: ParSeq[Point]): ParMap[Point, ParSeq[Point]] = {
    val pointsGroupByMeans = points.par.groupBy(findClosest(_, means))
    means.par.map(mean => mean -> pointsGroupByMeans.getOrElse(mean, ParSeq())).toMap
  }
