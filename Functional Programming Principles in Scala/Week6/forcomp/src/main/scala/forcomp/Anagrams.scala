...
object Anagrams extends AnagramsInterface {
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    y.foldLeft(x.toMap)((x_occs, y_occ) =>
      if(y_occ._2 == x_occs(y_occ._1)) x_occs - y_occ._1
      else x_occs updated(y_occ._1, x_occs(y_occ._1) - y_occ._2)
    ).toList
  }
      ...
      def occurrenceAnagrams(occurrences: Occurrences): List[Sentence] = {
        if(occurrences.isEmpty) List(List())
          ...
          for {
            ...
            if !words.isEmpty
            remaining <- occurrenceAnagrams(subtract(occurrences, comb))
