...
trait DecoderInstances {
  ...
  implicit val intDecoder: Decoder[Int] =
    Decoder.fromFunction {
      case Json.Num(n: BigDecimal) => if (n.isValidInt) Option(n.intValue) else None
      case other => None
    }
  ...
  implicit def listDecoder[A](implicit decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromPartialFunction {
      case Json.Arr(as) => as.flatMap(decoder.decode)
    }
  ...
  def field[A](name: String)(implicit decoder: Decoder[A]): Decoder[A] =
    Decoder.fromFunction {
      case Json.Obj(o) => decoder.decode(o.getOrElse(name, Json.Null))
    }
...
trait ContactsCodecs {
  ...
  implicit val contactsEncoder: Encoder[Contacts] =
    ObjectEncoder.field[String]("people")
      ...
      .transform[Contacts] (contact => ("people", contact.people))

  implicit val contactsDecoder: Decoder[Contacts] =
    Decoder.field[String]("people")
      .zip(Decoder.field[List[Person]]("people"))
      .transform[Contacts] (contacts => Contacts(contacts._2))
}
